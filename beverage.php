    ...
    <div class="container">
        ...
        <div class="row text-center py-5">
            <?php
                ...
                $result = $conn->query("SELECT `id`, `product_name`, `product_price`, `product_image`, `Category` FROM `Producttb` WHERE `Category` = 'Beverages'");
                while ($row = mysqli_fetch_assoc($result)){
                    component($row['product_name'], $row['product_price'], $row['product_image'], $row['id'],$row['Category="Beverages"']);
                }
            ?>
        </div>
</div>
